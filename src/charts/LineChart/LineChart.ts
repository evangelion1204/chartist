...
export function groupDataByAxisId(
  data: LineChartData
): Record<AxisId, LineChartData> {
  const groupedBy: Record<AxisId, LineChartData> = {
    y: { ...data, series: [] },
    y1: { ...data, series: [] }
  };

  each(data.series, series => {
    const id = (safeHasProperty(series, 'axisId') && series.axisId) || 'y';
    groupedBy[id].series.push(series);
  });
...
export class LineChart extends BaseChart<LineChartEventsTypes> {
  ...
  createChart(options: LineChartOptionsWithDefaults) {
    ...
    const createYAxis = (
      optionsY: AxisOptions,
      normalizedDataForYAxis: NormalizedData<NormalizedSeries[]>
    ): Axis => {
      if (optionsY.type === undefined) {
        return new AutoScaleAxis(
          axisUnits.y,
          normalizedDataForYAxis.series,
          chartRect,
          {
            ...optionsY,
    ...
    const dataGroupedByAxisId = groupDataByAxisId(data);

    const axisY = createYAxis(
      options.axisY,
      normalizeData(dataGroupedByAxisId.y, options.reverseData, true)
    );

    let axisY1: Axis | undefined = undefined;

    if (dataGroupedByAxisId.y1.series.length) {
      axisY1 = createYAxis(
        options.axisY1,
        normalizeData(dataGroupedByAxisId.y1, options.reverseData, true)
      );
    }
    ...
    if (axisY1) {
      axisY1.createGridAndLabels(
        gridGroup,
        labelGroup,
        {
          ...options,
          axisY: options.axisY1
        },
        this.eventEmitter
      );
    }
    ...
    each(
      ...
      (series, seriesIndex) => {
        ...
        const seriesMeta = safeHasProperty(series, 'meta')
          ? series.meta
          : undefined;
        const seriesAxisId: AxisId =
          (safeHasProperty(series, 'axisId') && series.axisId) || 'y';
        const activeAxisY = seriesAxisId === 'y1' && axisY1 ? axisY1 : axisY;
